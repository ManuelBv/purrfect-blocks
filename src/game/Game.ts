// Main game controller with drag-drop, audio, and animations
...
export class Game {
  ...
  private async processCascadeWithAnimation(): Promise<void> {
    ...
    if (totalLinesCleared > 0) {
      // Capture streak level BEFORE addScore increments it
      const streakLevelBefore = this.scoreManager.getStreakLevel();
      const score = this.scoreManager.addScore(totalLinesCleared);

      // Show streak message on 2nd+ consecutive clearing turn
      if (streakLevelBefore > 0) {
        const displayLevel = streakLevelBefore + 1;
        const multiplier = this.scoreManager.getComboMultiplier();
        this.soundEffects.playCascade(displayLevel);
        this.hudRenderer.showMessage(`${multiplier.toFixed(1)}x STREAK! +${score}`);
      }
    } else {
      // Reset streak if no lines cleared this turn
  ...
  private async saveGame(): Promise<void> {
    ...
    try {
      const state = {
        ...
        streakLevel: this.scoreManager.getStreakLevel(),
  ...
  private restoreGameState(savedGame: any): void {
    try {
      ...
      this.scoreManager['streakLevel'] = savedGame.streakLevel ?? 0;

      console.log('Game state restored: score =', savedGame.score, 'streak =', savedGame.streakLevel ?? 0);
    } catch (error) {
